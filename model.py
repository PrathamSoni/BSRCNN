...
class SRCNN(object):
  ...
  def build_model(self):
    ...
    #weights
    self.weights = {
      'w1': tf.Variable(tf.random_normal([5, 5, 9, 64], stddev=1e-3), name='w1'),
      'w2': tf.Variable(tf.random_normal([3, 3, 64, 32], stddev=1e-3), name='w2'),
      'w3': tf.Variable(tf.random_normal([1, 1, 32, 16], stddev=1e-3), name='w3'),
    ...
    if config.is_train:
      ...
      for ep in xrange(config.epoch):#for each epoch
        ...
        for idx in xrange(0, batch_idxs):
          ...
          average_ssim += ssim(self.pred.eval(feed_dict={self.images: batch_images, self.labels: batch_labels}), self.labels.eval(feed_dict={self.images: batch_images, self.labels: batch_labels}), multichannel=True)/batch_idxs
      ...
      image_path = os.path.join(os.getcwd(), "Test/Set5")
      ...
      CSRCNN=((io.imread("Test/Set5/test_image.png")[120:495, 120:495]).astype(float))
      print CSRCNN.dtype
      GT= ((io.imread("Test/Set5/rawimage-000361.bmp")[120:495, 120:495, 0]).astype(float))
      print GT.dtype

      print "PSNR: " + str(10*math.log10(255**2/(np.mean(np.square(GT-CSRCNN)))))
  def model(self):
